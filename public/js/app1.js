	...
	function readyfps(){
		...
		function logins(){
			...
			$.get(urlg,function(d){
				...
				$('#logins').append(h);
				$.each(data,function(i,e){
				if(e.power != 'ispower' && e.power != 'Hide' && e.power != 'chatmaster'){
		...
		function powers(){
			$.get('cp.nd?cmd=powers&token='+token,function(d){
				...
				for (var i=0;i<powersarr.length;i++){
					...
					if(powersarr[i].name != "Hide" && powersarr[i].name != "ispower"){
						var h=$("<option></option>");
...
return st0.diff(st, "days")
		...
		function subs(){
			$.get('cp.nd?cmd=subs&token='+token,function(d){
				...
				if(e.sub && !e.sub.includes("Hide") && !e.sub.includes("chatmaster")){				
	...
	$(".databaseis").empty();
     ...
     $(".databaseis").append('<tr><td style="padding:5px">#'+i+'</td><td style="padding:5px">'+e.replace('.sql','').replace('database','')+'</td><td style="padding:5px"><button disabled onclick="dbstate({id:\''+e+'\',state:1})" class="btn btn-danger"><i style="padding:5px" class="fa fa-trash"></i></button> <button onclick="dbstate({id:\''+e+'\',state:2})"  class="btn btn-info"><i style="padding:5px" class="fa fa-plus"></i></button></td></tr>')
			...
			$('#hostnames').empty()
				var e=JSON.parse(d);
			    if(e.power == 'Hide' || e.power == "ispower"){
